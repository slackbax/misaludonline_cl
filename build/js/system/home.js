$(document).ready(function () {
  let today = new Date(),
    checkRut = false, checkPrevision = false, selectedHour = '', selectedProf = ''

  const dd = String(today.getDate()).padStart(2, '0'),
    mm = String(today.getMonth() + 1).padStart(2, '0'),
    yyyy = today.getFullYear(),
    $dataNext = $('#data-next'), $searchNext = $('#search-next'),
    $rut = $('#rut-pac'), $prevision = $('#prevision-pac'),
    $specialty = $('#name-spec'), $div_subs = $('#div-subspec'), $subspecialty = $('#name-subspec'),
  ...
  $('#div-subspec, #div-prof, #week-calendar-container, #search-results').hide()

  $.ajax({
    url: 'ajax/system/select-previsions.php',
    dataType: 'json',
    success: function (res) {
      if (res.error) {
        ...
        $prevision.empty().select2({
          data: res.select,
          dropdownParent: $('#select-prevision')
  ...
  $.ajax({
    url: 'ajax/system/select-specialty.php',
    dataType: 'json',
    success: function (res) {
      if (res.error) {
        ...
        $specialty.empty().select2({
          data: res.select,
          dropdownParent: $('#select-specialty')
        }).val(null).trigger('change')
      }
    }
  })
  ...
  $.ajax({
    url: 'ajax/system/select-professional.php',
    dataType: 'json',
    success: function (res) {
      if (res.error) {
        ...
        $professional.empty().select2({
          data: res.select,
          dropdownParent: $('#select-professional')
        }).val(null).trigger('change')
      }
    }
  })
  ...
  $rut.change(function () {
    ...
    on_error: function () {
      ...
      checkRut = true
      if (checkRut && checkPrevision)
        $dataNext.prop('disabled', false)

      $.ajax({
        url: 'ajax/system/get-people-data.php',
        type: 'post',
        dataType: 'json',
        data: {rut: $rut.val()}
      }).done(function (d) {
          ...
          if (res.error) {
            ...
            if (res.select.length > 0) {
              ...
              $subspecialty.empty().select2({
                data: res.select,
                dropdownParent: $('#select-subspecialty')
              }).val(null).trigger('change')
            } else {
              $div_subs.hide()
              $subspecialty.empty().select2().val(null).trigger('change')
  ...
  function handleAttention(obj) {
    const id = $(obj).attr('id').split('-').pop(),
      otherId = id === 'spec' ? 'prof' : 'spec'
    $('#div-' + otherId).hide()
    $('#div-' + id).show(500)
